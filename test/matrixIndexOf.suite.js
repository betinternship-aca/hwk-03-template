...
suite('matrixIndexOf', () => {
    ...
    require('../solutions/matrixIndexOf');
    const code = fs.readFileSync('solutions/matrixIndexOf.js', {encoding: 'utf8'});
    ...
    test(`should return null if the item doesn't exist in the given matrix`, () => {
        expect(matrixIndexOf(matrix, value)).to.be.null;
    });

    test('should return the index of item in the matrix as an array or an object', () => {
        setValueAt(firstCoord);

        const result = checkResultAndFix(matrixIndexOf(matrix, value));

        expect(result).to.deep.equal(firstCoord);
    });

    test('should return the first index of the item if it appears in the matrix more then once', () => {
        setValueAt(firstCoord);
    ...
    test(`you should use your function for arrays`, () => {
        expect(/indexOf\s*\(/.test(code)).to.be.true;
    });
});
